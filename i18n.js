...
(function () {
  var DICT = {
    ja: {
      ...
      priv_s2_p: "「SigNote広場」にログインする際、<strong>Sign in with Apple</strong> または <strong>Google</strong> を通じてユーザー識別子を取得します。ログイン方法と設定により、メールアドレスや表示名が含まれる場合があります。パスワードを取得することはありません。広場で表示される<strong>作者名は自分で決めるニックネーム</strong>（初期値はランダムな仮名）で、Apple／Google の実名は表示されません。用途：投稿・いいねの識別、アカウントの維持。閲覧とダウンロードにログインは不要です。",
    ...
    en: {
      ...
      priv_s2_p: "When you sign in to SigNote Plaza, we receive a user identifier via <strong>Sign in with Apple</strong> or <strong>Google</strong>; depending on the method and your choices this may include your email address or display name. We never receive your password. Your <strong>creator name is a nickname you choose</strong> (a random alias by default) — your real Apple/Google name is never displayed. Purpose: identifying your uploads and likes and maintaining your account. Browsing and downloading do not require sign-in.",
